    ...
    <li className="flex flex-col gap-4 py-6 sm:flex-row sm:items-center sm:justify-between">
      <div className="flex items-center gap-6">
        <div className="w-16 shrink-0 text-center">
          ...
          <p className="text-[11px] text-cream/40">{d.getFullYear()}</p>
        </div>
        <div>
          ...
          <p className="mt-1 text-[11px] tracking-widest text-cream/40 uppercase">
            {show.eventType}
          </p>
        </div>
      </div>
      <div className="flex flex-wrap items-center gap-3 sm:justify-end">
        <a
          ...
          className="text-xs tracking-widest text-cream/50 uppercase transition hover:text-gold"
        >
          + Google Cal
        </a>
        <a
        ...
        {show.soldOut ? (
          <span className="text-sm font-semibold tracking-widest text-cream/40 uppercase">
            Sold out
          </span>
        ) : show.ticketUrl ? (
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="bg-gradient-brand rounded-full px-6 py-2.5 text-sm font-semibold tracking-wide text-ink uppercase transition hover:opacity-90"
          >
            Tickets
          </a>
        ) : (
    ...
    <div>
      <div className="mb-4 flex items-center justify-between">
        <button
          ...
          className="rounded-full border border-cream/20 px-4 py-1.5 text-sm transition hover:border-clay hover:text-clay"
      ...
      <div className="grid grid-cols-7 gap-px overflow-hidden rounded-xl border border-cream/10 bg-cream/10">
        {["S", "M", "T", "W", "T", "F", "S"].map((d, i) => (
          <div key={i} className="bg-ink p-2 text-center text-xs text-cream/40">
            {d}
          </div>
        ))}
        {cells.map((day, i) => (
          <div key={i} className="min-h-20 bg-ink p-1.5">
            {day && (
              <>
                <p className="text-xs text-cream/40">{day}</p>
                {(byDay.get(day) ?? []).map((s) => (
                  <a
                    ...
                    rel="noopener noreferrer"
                    className="bg-gradient-brand mt-1 block truncate rounded px-1.5 py-1 text-[10px] leading-tight font-semibold text-ink"
    ...
    <div>
          ...
          {(["list", "calendar"] as const).map((v) => (
            <button
              ...
              className={`rounded-full px-5 py-1.5 text-sm capitalize transition ${
                view === v ? "bg-cream/10 text-cream" : "text-cream/50 hover:text-cream"
              }`}
            >
              {v}
            </button>
      ...
      <div className="mt-8">
        {view === "list" ? (
          upcoming.length > 0 ? (
            ...
            <div className="rounded-2xl border border-dashed border-cream/20 px-6 py-16 text-center">
      ...
      {past.length > 0 && (
        <section className="mt-16">
          <h2 className="font-display text-2xl font-bold text-cream/70">Past shows</h2>
          <ul className="mt-4 space-y-2">
            {past.map((s) => {
              const d = toDate(s.dateTime);
              return (
                <li key={s.dateTime + s.venueName} className="text-sm text-cream/50">
